...
static void collapse_all(struct Context *ctx, struct Menu *menu, int toggle)
{
  ...
  menu_queue_redraw(menu, MENU_REDRAW_FULL | MENU_REDRAW_STATUS);
}
  ...
  while (true)
  {
    ...
    switch (op)
    {
      ...
      case OP_RECONSTRUCT_THREAD:
      {
        ...
        if (shared->mailbox->msg_count > oldmsgcount)
        {
          ...
          if (priv->in_pager)
          {
            ...
            menu_queue_redraw(priv->menu, MENU_REDRAW_STATUS | MENU_REDRAW_FULL);
      ...
      case OP_MAIN_DELETE_PATTERN:
        ...
        menu_queue_redraw(priv->menu, MENU_REDRAW_FULL | MENU_REDRAW_STATUS);
        break;
      ...
      case OP_NEXT_ENTRY:
      {
        ...
        if (priv->in_pager)
        {
          ...
          menu_queue_redraw(priv->menu, MENU_REDRAW_OLD_CUR);
        break;
      }
      ...
      case OP_PIPE:
      {
        ...
        {
          menu_queue_redraw(priv->menu, (priv->tag ? MENU_REDRAW_FULL : MENU_REDRAW_CURRENT) |
                                            MENU_REDRAW_STATUS);
        }
#endif
        break;
